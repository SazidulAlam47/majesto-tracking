  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      if (result.data?.status === 'approved' && result.data?.token) {
        ...
        toast.info('Your request is still pending. Please wait for admin approval.');
      } else if (result.data?.status === 'rejected') {
        toast.error('Your request was rejected. Please contact the admin.');
        safeStorage.removeItem('login_request');
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400">
                      Hi <span className="text-white font-medium">{loginRequest.name}</span>,
